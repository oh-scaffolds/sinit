...
exports.cloneGitScaffold = cloneGitScaffold;
exports.copyScaffold = copyScaffold;
...
function builder(yargs) {
    return yargs.option('e', {
        alias: 'encoding',
        describe: 'Encoding of files in scaffold',
        type: 'string',
        nargs: 1,
        default: 'utf8'
    }).usage('Usage: $0 create <scaffold> <source>').demand(3).showHelpOnFail(true, "Specify --help for available options");
}
...
function handler(argv) {
    ...
    if (!source) {
        (0, _log.fatal)('Source git repository or folder is required.');
    }
        ...
        if ((0, _isUrl2.default)(source)) {
            ...
            cloneGitScaffold(source, dest, {
                ...
                if (err) {
                    ...
                    _const2.default.cfgFile.set(scaffold, 'source', source);
...
function copyScaffold(src, dest, opt, callback) {
    ...
    var ignores = ((0, _util.readFileNoErr)(_path2.default.join(src, _const2.default.ignoreFile), encoding) || '').split('\n').map(function (p) {
        return p.replace(/\/$/, '');
    }).concat('.git');
